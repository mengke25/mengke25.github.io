...
program define dynamic_fig
    ...
    syntax, y(varlist) treat(varlist) time(varlist) ref(integer) absorb(string) [condition(string)] cluster(varlist) [cov(string)] [level(string)] [treattype(string)] [figname(string)] [figtitle(string)] [figsubtitle(string)]  [regtype(string)]  
	...
	if "`treattype'" == "intensity"{
		...
		// Execute the regression with or without condition
		if "`condition'" != "" {
			di "TWFE: `regtype'hdfe `y' c.`treat'#i(`coef_t').`time' `cov' if `condition', absorb(`absorb') cluster(`cluster')"
			`regtype'hdfe `y' c.`treat'#i(`coef_t').`time' `cov' if `condition', absorb(`absorb') cluster(`cluster') level(`level')
		}
		else {
			di "TWFE: `regtype'hdfe `y' c.`treat'#i(`coef_t').`time' `cov', absorb(`absorb') cluster(`cluster')"
			`regtype'hdfe `y' c.`treat'#i(`coef_t').`time' `cov', absorb(`absorb') cluster(`cluster')  level(`level')
		}
	}
	...
	if `gap' != 0{
		...
		// Execute the regression with or without condition
		if "`condition'" != "" {
			di "TWFE: `regtype'hdfe `y' c.`treat'#i(`coef_k').`time' `cov' if `condition', absorb(`absorb') cluster(`cluster')"
			`regtype'hdfe `y' c.`treat'#i(`coef_t').`time' `cov' if `condition', absorb(`absorb') cluster(`cluster') level(`level')
		}
		else {
			di "TWFE: `regtype'hdfe `y' c.`treat'#i(`coef_k').`time' `cov', absorb(`absorb') cluster(`cluster')"
			`regtype'hdfe `y' c.`treat'#i(`coef_t').`time' `cov', absorb(`absorb') cluster(`cluster')  level(`level')
		}
	}
	}
